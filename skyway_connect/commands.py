...
from skyway_connect.extensions import db
from skyway_connect.user.models import User
...
def install():
    ...
    from skyway_connect.user.models import Role
    ...
    if a is None:
        ...
        try:
            db.session.add(r)
            db.session.commit()
            u = click.prompt('Admin Email?', default='admin@skyway.com')
            p = click.prompt('Admin Password (min 6 characters)?', default='skyway')
